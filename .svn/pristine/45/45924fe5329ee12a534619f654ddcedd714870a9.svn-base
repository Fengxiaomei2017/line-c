<template>
  <div>
    <header class="mint-header is-fixed">
      <a class="mint-header-button is-left" @click="goBack">
        <i class="mintui mintui-back"></i>
      </a>
      <span class="mint-header">确认订单</span>
      <div class="mint-header-button is-right">
      </div>
    </header>
    <div class="content">
      <div class="content-cell">
        <div class="cell-title">
          预订信息
        </div>
        <div class="cell-body">
          <!-- <div class="w-40">
            <div class="hotelImg" :style="{backgroundImage: 'url('+img+')'}"></div>
          </div> -->
          <img class="w-40" :src="img">
          <div class="w-60">
            <div class="product-name">{{productName}}</div>
            <div class="seller-name font-gray">{{sellerName}}</div>
          </div>
        </div>
        <!-- <div class="text-right"></div> -->
        <div class="order-subtitle">
          <span class="subtitle-item">出行方案：
            <span class="font-gray subtitle-content">{{stockGoodsName}}</span></span>
          <span class="subtitle-item">出行时间：
            <span class="font-gray subtitle-content">{{day}}</span></span>
          <span class="subtitle-item">出行人数：
            <span class="font-gray subtitle-content">{{adult}}成人 <span v-if="child > 0">{{child}}儿童</span></span></span>
        </div>
      </div>
      <div class="content-cell">
        <div class="cell-title">联系人</div>
        <mt-field class="contactPerson" label="姓名:" :state="contactPersonState" placeholder="请输入真实姓名"
                  v-model="contactPerson"></mt-field>
        <mt-field label="手机号:" placeholder="用于紧急联系和接受行程信息" :state="contactPhoneState" type="tel"
                  v-model="contactPhone"></mt-field>
      </div>
      <div class="content-cell">
        <div class="cell-title">开具发票说明</div>
        <p class="font-12">若需要开具发票，请直接联系供应商。发票金额不包含已优惠部分金额和保险金额。</p>
      </div>
      <div class="black-bottom"></div>
    </div>
    <div class="fixed-bottom">
      <div class="must font-12">
        <input
          type="checkbox"
          :class="{'input-uncheck': isUncheck}"
          @click="agree"/><span>同意</span>
        <a href="javasript:">预定须知和重要条款</a>
      </div>
      <div class="bottom">
        <div class="bg-white yellow total">合计：￥
          <span class="font-weight-bold price-num">{{orderMoney}}</span></div>
        <!-- <button class="next-step" :disabled="disabled">提交订单</button> -->
        <mt-button
          class="next-step"
          @click="createOrder"
          :disabled="disabled">提交订单</mt-button>
      </div>
    </div>

    <mt-popup
      v-model="popupVisiblePayWays"
      position="bottom" class="popup-pay">
      <PayCom
        :orderNo="orderNo"
        :stockGoodsId="stockGoodsId"
        :stockGoodsName="stockGoodsName"
        :adult="adult"
        :child="child"
        :orderMoney="orderMoney"
        :day="day"
        v-on:watchOrder="_closeOrder"></PayCom>
    </mt-popup>
    <mt-popup
      v-model="popupVisiblePayWait"
      class="popup-wait">
      <div class="prpup-wait-title">请确认微信支付是否已完成</div>
      <a href="javasript:"
         class="pay-finished"
         @click="finished">已完成支付</a>
      <a href="javascript:" class="font-gray" @click="_hidePayFinish">支付遇到问题，重新支付</a>
    </mt-popup>
  </div>
</template>

<script>
//  import { Navbar, TabItem } from 'mint-ui'
//  Vue.component(Navbar.name, Navbar)
import {getStore} from '@/config/mUtils'
import {createOrder} from '@/api/getData'
import {mapMutations} from 'vuex'

import PayCom from '@/components/pay'
import Vue from 'vue'
// import {Popup, Field, MessageBox, Button} from 'mint-ui'
import {Popup, Field, Button} from 'mint-ui'

Vue.component(Field.name, Field)
Vue.component(Popup.name, Popup)
Vue.component(PayCom.name, PayCom)
Vue.component(Button.name, Button)

export default {
  name: 'OrderConfirm',
  data () {
    return {
      popupVisiblePayWays: false,
      popupVisiblePayWait: false,
      img: '',
      stockGoodsId: '',
      stockGoodsName: '',
      sellerName: '',
      adult: 0,
      child: 0,
      orderNo: '',
      orderMoney: 0,
      day: '',
      contactPerson: '',
      contactPersonState: '',
      contactPhone: '',
      contactPhoneState: '',
      buyerId: 1, // 用户ID
      isUncheck: true,
      disabled: true
    }
  },
  methods: {
    ...mapMutations([
      'PAY_FINISH'
    ]),
    agree () {
      this.isUncheck = !this.isUncheck
      this.disabled = !this.disabled
      // if (this.isUncheck) {
      //   this.isUncheck = false
      //   this.disabled = false
      // } else {
      //   this.isUncheck = true
      //   this.disabled = true
      // }
    },
    goBack () {
      this.$router.push({name: 'ChooseDate', params: {id: this.productId}})
    },
    selectedPayWay (wayId) {
      this.payWay = wayId
    },
    closePopup () {
      this.popupVisiblePayWays = false
    },
    checkPhoneNum (value) {
      return /^(1[0-9][0-9]|14[57]|15[012356789]|17[0678]|18[0-9])\d{8}$/.test(value)
    },
    createOrder () {
      // this._hidePayFinish()
      if (!this.orderNo) {
        this.contactPersonState = this.contactPerson === '' ? 'error' : 'success'
        this.contactPhoneState = this.checkPhoneNum(this.contactPhone) ? 'success' : 'error'
        if (this.contactPersonState === 'error' || this.contactPhoneState === 'error') {
          return
        }
        // if (this.$store.state.lineUser.id) {
        createOrder(this.stockGoodsId, this.adult, this.child, this.orderMoney, this.$store.state.lineUser.id, this.contactPerson, this.contactPhone).then(
          res => {
            if (res.data.state === 1) {
              // this.MessageBox('提示', res.data.message);
              this.orderNo = res.data.data.orderNo
              this.popupVisiblePayWays = true
              // 如果创建成果跳转支付页面
            } else {
              // MessageBox('提示', res.data.message)
            }
          }
        )
        // }
        // else {
        //   this.$router.push({name: 'Login'})
        // }
      } else {
        this.popupVisiblePayWays = true
      }
    },
    finished () {
      let orderNo = this.$store.state.payFinish.orderNo
      this.$router.push({path: '/order-details', query: {id: orderNo}})
      this.PAY_FINISH()
    },
    _closeOrder () {
      this.popupVisiblePayWays = false
    },
    _hidePayFinish () {
      // let ua2 = navigator.userAgent.toLowerCase()
      // let result2 = ua2.match(/MicroMessenger/i) || ''
      // if (!result2) {
      this.PAY_FINISH()
      this.popupVisiblePayWait = false
      // }
    }
    // payThisOrder () {
    //   payOrder(this.orderNo).then(
    //     res => {
    //       if (res.data.state === 1) {
    //         MessageBox('提示', res.data.message)
    //       } else {
    //         // MessageBox('提示', res.data.message)
    //       }
    //     }
    //   )
    // }
  },
  created () {
    let product = getStore('BOOK-PRODUCT')
    if (product) {
      product = JSON.parse(product)
    }
    let bookInfo = this.$route.query
    // debugger;
    // this.data = product.assign(bookInfo);
    this.stockGoodsId = bookInfo.stockGoodsId
    this.stockGoodsName = bookInfo.stockGoodsName
    this.adult = bookInfo.adult
    this.child = bookInfo.child
    this.orderMoney = bookInfo.orderMoney
    this.day = bookInfo.day
    this.productId = product.productId
    this.productName = product.productName
    this.img = product.img
    this.sellerName = product.sellerName
    if (JSON.stringify(this.$store.state.payFinish) !== '{}') {
      let ua2 = navigator.userAgent.toLowerCase()
      let result2 = ua2.match(/MicroMessenger/i) || ''
      if (!result2) {
        this.popupVisiblePayWait = true
      // console.log(this.$store.state.payFinish.orderNo)
      }
    }
  }
  // let ua2 = navigator.userAgent.toLowerCase()
  // let result2 = ua2.match(/MicroMessenger/i) || ''
  // if (!result2 && JSON.stringify(this.$store.state.payFinish) !== '{}') {
  //   alert(window.location.href)
  //   this.popupVisiblePayWait = true
  // }
}
</script>

<style lang="less" type="text/less" rel="stylesheet/less" scoped>
  @import "../style/main.less";

  .black-bottom {
    .px2rem(height, 90);
  }

  .cell-body {
    width: 100%;
    height: 5rem;
    .px2rem(margin-bottom, 10);
    .w-40 {
      display: inline-block;
      height: 5rem;
      vertical-align: top;
      /*padding: 13px;*/
      .px2rem(width, 130);
      /*.px2rem(padding-left, 10);*/
      /*.hotelImg {
        background-image: url(../assets/timg.jpg);
        height: 5rem;
        background-size: cover;
      }*/
    }
    .w-60 {
      display: inline-block;
      /*float: right;*/
      .px2rem(width, 200);
      .px2rem(margin-left, 5);
      .product-name {
        max-height: 3rem;
        overflow: hidden;
        font-size: @font-16;
      }
      .seller-name {
        .px2rem(height, 30);
        .px2rem(line-height, 30);
        font-size: @font-14;
      }
    }
  }

  .order-subtitle {
    .subtitle-item {
      display: inline-block;
      .px2rem(line-height, 30);
      .px2rem(margin-right, 25);
      font-size: @font-15;
      .subtitle-content {
        font-size: @font-14;
      }
    }
  }

  .btn-yellow {
    color: #fff;
    background-color: @yellow;
    border-color: @yellow;
  }

  .cell-title {
    border-bottom: 0 !important;
  }

  /*.content {*/
  /*position: absolute;*/
  /*top: 2.5rem;*/
  /*bottom: 0;*/
  /*width: 100%;*/
  /*left: 0rem;*/
  /*overflow: auto;*/
  /*background-color: #eee;*/

  /*.content-cell {*/
  /*background-color: #fff;*/
  /*min-height: 2rem;*/
  /*margin-top: 0.5rem;*/
  /*padding: 0.5rem;*/

  /*.row + .row {*/
  /*margin-top: 0.5rem;*/
  /*}*/

  /*}*/

  /*}*/
  .contactPerson {
    border-bottom: 1px solid #c3c3c3;
    .px2rem(border-width, 0.5)
  }

  .fixed-bottom {
    .must {
      .px2rem(line-height, 30);
      background-color: rgba(220, 220, 220, 0.5);
      input {
        .px2rem(width, 20);
        .px2rem(height, 20);
        .px2rem(margin-left, 20);
        .px2rem(margin-right, 5);
        vertical-align: -15%;
        border-radius: 4px;
        box-shadow: 2px 2px 3px #fff inset;
        &.input-uncheck {
          -webkit-appearance: none;
          background-color: #fff;
        }
      }
    }
    .bottom {
      display: flex;
      text-align: center;
      .total {
        width: 50%;
        /*flex: 0 0 20%;*/
        display: inline-block;
        .px2rem(height, 50);
        .px2rem(line-height, 50);
        border-top: 1px solid #e5e5e5;
        .px2rem(border-top-width, 0.5);
        .price-num {
          .px2rem(font-size, 25);
        }
      }
      .next-step {
        width: 50%;
        /*display: inline-block;*/
        background: @orange;
        color: white;
        .px2rem(height, 50);
        .px2rem(line-height, 50);
        border: 1px solid @orange;
      }
    }
  }

  .popup-pay {
    width: 100%;
    /*.px2rem(height, 120);
    .px2rem(line-height, 60);
    font-size: @font-18;
    text-align: center; */
  }

  .popup-wait {
    width: 80%;
    .px2rem(height, 200);
    .px2rem(border-radius, 5);
    text-align: center;
    .prpup-wait-title {
      .px2rem(height, 70);
      .px2rem(line-height, 70);
      color: #000;
    }
    a {
      display: block;
      .px2rem(height, 60);
      .px2rem(line-height, 60);
      border-top: 1px solid #e5e6e6;
      text-decoration: none;
      &.pay-finished {
        color: #fe5066;
      }
    }
  }

  .p-payway-list {
    padding: 0 0.5rem;
    font-size: 0.85rem;
    li {
      position: relative;
      padding: 0.6rem 0.6rem 0.6rem 0;
      border-bottom: 1px solid #F2F5F8;
      overflow: hidden;
      min-height: 2rem;
      display: table;
      width: 100%;
    }
    .icon {
      width: 1.5rem;
      height: 1.5rem;
      text-align: center;
      display: table-cell;
      vertical-align: middle;
    }
    .ro {
      display: table-cell;
      width: 1.5rem;
      height: 1.5rem;
      text-align: center;
      vertical-align: middle;
      /*input{*/
      /*display: none;*/
      /*}*/
      .mint-radio-input:checked + .mint-radio-core {
        background-color: @blue;
        border-color: @blue;
      }
    }
    i {
      display: inline-block;
      width: 1rem;
      height: 1rem;
      vertical-align: top;
      background-size: 1rem 1rem;
      background-repeat: no-repeat;
      vertical-align: middle;
      &.pay_2 {
        background: url(data:image/svg+xml;base64,);
        background-size: 2rem 2rem;
        background-position: -2.2rem -2.4rem;
        margin-top: 0.3rem;
      }
      &.pay_1 {
        background: url(data:image/svg+xml;base64,);
        margin-top: 0.05rem;
      }
      &.pay_3 {
        background-image: url(data:image/svg+xml;base64,);
      }
      &.pay_3 {
        background-image: url(data:image/svg+xml;base64,);
      }
      &.pay_4 {
        background-image: url(data:image/svg+xml;base64,);
        background-position: 0px 2px;
        height: 1.1rem;
      }
    }
  }
</style>
