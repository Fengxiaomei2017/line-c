...
<script>
import {payOrder, wxForward} from '@/api/getData'
...
export default {
  ...
  methods: {
    ...
    goHome () {
      ...
      this.$router.push({path: '/order-details', query: {id: this.orderNo}})
      this.$emit('watchOrder')
    },
    jsApiCall () {
      let $this = this
      WeixinJSBridge.invoke(        // eslint-disable-line
        ...
        function (res) {
          ...
          if (res.err_msg === 'get_brand_wcpay_request:ok') {
            ...
            // 支付成功
            // title提示框的标题	String
            // message提示框的内容	String
            // showConfirmButton是否显示确认按钮	Boolean		true
            // showCancelButton是否显示取消按钮	Boolean		false
            // confirmButtonText
            // MessageBox({
            //   title: '提示',
            ...
            // MessageBox('错误', JSON.stringify(res))
            // alert(JSON.stringify(res))
    ...
    pay () {
      ...
      if (result) {
        if (result.join(',') === 'micromessenger') {
          ...
          payOrder(this.orderNo).then(
            res => {
              if (res.data.state === 1) {
                // MessageBox('提示', res.data.message)
                // MessageBox('成功', JSON.stringify(res));
                let data = res.data.data
                this.payInfo.timeStamp = data.timeStamp
        ...
        // 独立浏览器支付
        wxForward(this.orderNo).then(res => {
          Indicator.close()
          let wxRedirectUrl = 'http://www.daxi51.com/#/order-confirmation?stockGoodsId=' + this.stockGoodsId + '&stockGoodsName=' + this.stockGoodsName + '&adult=' + this.adult + '&child=' + this.child + '&orderMoney=' + this.orderMoney + '&day=' + this.day
          let encodeWxRedirectUrl = encodeURIComponent(wxRedirectUrl)
          if (res.data.state === 1) {
            let callbackPage = res.data.data.redirectUrl + '&redirect_url=' + encodeWxRedirectUrl
            window.location.href = callbackPage
            let payFinish = {orderNo: this.orderNo}
            this.PAY_FINISH(payFinish)
          }
        })
      }
    }
