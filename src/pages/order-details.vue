    ...
    <div class="content">
      <div class="content-cell m-0 p-0">
        <div class="float-right m-2">
          <button
            ...
            v-if="orderStateCfg[order.orderState].options"
      ...
      <div class="content-cell">
        ...
        <div class="cell-body">
          ...
          <div v-if="order.orderType === 1 ">下单时间： {{order.orderTime}}</div>
          <div v-if="order.orderType === 2 ">下单时间： {{order.orderTimeStr}}</div>
          <div>支付方式： 微信支付</div>
      ...
      <div class="content-cell">
        ...
        <div class="cell-body" v-if="order.orderType === 1">
          ...
          <div>出行人数： {{order.orderDetailInfoVo.adultNumber}}成人 <span v-if="order.orderDetailInfoVo.childNumber > 0">{{order.orderDetailInfoVo.childNumber}}儿童</span></div>
        </div>
        <div class="cell-body" v-else>
          <div>{{order.orderName}}</div>
          <div>{{order.hotelName}}</div>
          <div>房间数： {{order.goodsNum}}间房&nbsp;&nbsp;&nbsp;
              住{{order.night}}晚</div>
          <!-- <div>出行人数： </div> -->
        </div>
      </div>
      <div class="content-cell">
        ...
        <div class="cell-body" v-else>
          <div>姓名： {{order.checkinPerson}}</div>
          <div>手机号： {{order.contactMobile}}</div>
        </div>
      </div>
    </div>
    <mt-popup
        ...
        getOrderDetail(this.$store.state.lineUser.id, this.orderNo).then(res => {
          if (res.data.state === 1) {
            ...
            if (this.order.orderType === 1) {
              ...
              this.timeList.push({
                content: '订单提交成功',
                theTime: this.order.orderTimeStr,
                icon: '√'
              })
            }
            //如果创建成果跳转支付页面
