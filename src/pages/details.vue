    ...
    <div v-show="isContent">
          ...
          <div class="cell-body font-12">
            <div class="comment-header">
              ...
              <div class="comment-score">
                ...
                <span class="text-danger">{{evaluateList.scoreText}}</span>
              </div>
            </div>
            <div class="comment-content">
              {{evaluateList.content}}
              <div class="img-list"
                   v-if="evaluateList.evaluateImages">
      ...
      <div class="fixed-bottom">
                ...
                style="z-index: 10000;">
                  ...
                  <div class="scroll-box">
                    <ul class="all-days" v-if="product.planMode === 'detail'">
                      <li
                        class="every-day all-day-text"
                        v-if="allLine.length"
                        @click="_clickDay(-1)">全部</li>
                      <li class="every-day"
                          v-for="(item,indexDay) in days"
                          v-if="product.singlePlan[indexDay].destination.length"
        ...
        <div class="bottom">
          ...
          <div
            v-if="product.remainingNumber > 0"
            class="next-step"
            @click="book">低至￥
  ...
  methods: {
    ...
    getDetail () {
      getProductById(this.productId).then(res => {
        if (res && res.data.state === 1) {
          ...
          // console.log(this.product.evaluateList)
    ...
    _sentMapData (newSinglePlan) {
      ...
      if (this.day === 0) {
        ...
        if (this.markLine.length) {
          this.allLine = this.markLine
        }
      } else {
        let currentDay = this.day - 1
        if (newSinglePlan[currentDay].destination.length) {
          for (var j = 0; j < newSinglePlan[currentDay].destination.length; j++) {
            var lngLatOld = newSinglePlan[currentDay].destination[j].lngLat
            var address = newSinglePlan[currentDay].destination[j].address
            var content = newSinglePlan[currentDay].destination[j].content
